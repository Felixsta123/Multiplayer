...
AImprovedVoxelBuilding::AImprovedVoxelBuilding()
{
    ...
    // Stairs building properties
    bIsStairsBuilding = false;
    StairsDirection = 0;
...
void AImprovedVoxelBuilding::BeginPlay()
{
    ...
    {
        // We want to make sure stairs buildings are correctly generated on clients too
        if (GetLocalRole() == ROLE_Authority || bIsStairsBuilding)
        {
            GenerateBuilding();
        }
    }
...
void AImprovedVoxelBuilding::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
    ...
    // Add the new stairs properties
    DOREPLIFETIME(AImprovedVoxelBuilding, bIsStairsBuilding);
    DOREPLIFETIME(AImprovedVoxelBuilding, StairsDirection);

    // Replicate the destruction history
    DOREPLIFETIME(AImprovedVoxelBuilding, DestructionHistory);
}

void AImprovedVoxelBuilding::OnRep_IsStairsBuilding()
{
    // When stairs building flag is replicated to client, regenerate the building
    if (bIsStairsBuilding)
    {
        UE_LOG(LogTemp, Warning, TEXT("OnRep_IsStairsBuilding: Regenerating stairs building with direction %d"), StairsDirection);

        // Only regenerate if we're not on the server (clients only)
...
{
    // Initialize voxel grid based on building type
    if (bIsStairsBuilding)
    {
        GenerateStairsBuilding();
    }
    else
    {
        InitializeVoxelGrid();
...
void AImprovedVoxelBuilding::GenerateStairsBuilding()
{
    UE_LOG(LogTemp, Warning, TEXT("Generating stairs building with direction: %d"), StairsDirection);
    ...
    // Calculate the step height and width
    int32 NumSteps = FMath::Min(GridSizeX, GridSizeZ);

    // Generate stairs based on direction
    switch (StairsDirection % 4)
    {
        case 0: // East to West (X decreasing, Z increasing)
            for (int32 Step = 0; Step < NumSteps; Step++)
            {
                int32 X = GridSizeX - 1 - Step;
                if (X < 0) continue;

                for (int32 Z = 0; Z <= Step; Z++)
                {
                    if (Z >= GridSizeZ) continue;
        ...
        case 1: // West to East (X increasing, Z increasing)
            for (int32 Step = 0; Step < NumSteps; Step++)
            {
                int32 X = Step;
                if (X >= GridSizeX) continue;
        ...
        case 2: // North to South (Y decreasing, Z increasing)
            for (int32 Step = 0; Step < NumSteps; Step++)
            {
                int32 Y = GridSizeY - 1 - Step;
                if (Y < 0) continue;
        ...
        case 3: // South to North (Y increasing, Z increasing)
            for (int32 Step = 0; Step < NumSteps; Step++)
            {
                int32 Y = Step;
                if (Y >= GridSizeY) continue;
    ...
    UE_LOG(LogTemp, Warning, TEXT("Stairs building generated successfully"));
}
